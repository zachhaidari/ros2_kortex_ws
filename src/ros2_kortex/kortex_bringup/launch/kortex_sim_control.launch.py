...
def launch_setup(context, *args, **kwargs):
    ...
    # Build the world file path
    world_file = world.perform(context)
    if world_file and world_file != "empty.sdf":
        # Use custom world from kortex_bringup package
        world_path = PathJoinSubstitution(
            [FindPackageShare("kortex_bringup"), "worlds", world_file]
        ).perform(context)
        gz_world_arg = f" -r -v 3 {world_path} --physics-engine gz-physics-bullet-featherstone-plugin"
    else:
        gz_world_arg = " -r -v 3 empty.sdf --physics-engine gz-physics-bullet-featherstone-plugin"
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            description="Gazebo world file (e.g., pick_and_place.sdf)",
        )
    )
